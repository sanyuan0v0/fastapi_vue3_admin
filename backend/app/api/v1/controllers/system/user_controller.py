...
async def get_current_user_info(
    ...
    result_dict = await UserService.get_current_user_info(auth=auth)
...
async def user_avatar_upload(
    ...
    result_str = await UserService.upload_avatar(request=request, file=file)
...
async def update_current_user_info(
    ...
    result_dict = await UserService.update_current_user_info(data=data, auth=auth)
...
async def change_current_user_password(
    ...
    result_dict = await UserService.change_user_password(data=data, auth=auth)
...
async def register_user(
    ...
    user_register_result = await UserService.register_user(data=data, auth=auth)
...
async def forget_password(
    ...
    auth = AuthSchema(db=db)
    user_forget_password_result = await UserService.forget_password(data=data, auth=auth)
...
async def get_obj_list(
    ...
    result_dict_list = await UserService.get_user_list(search=search, auth=auth, order_by=page.order_by)
...
async def get_obj_detail(
    ...
    result_dict = await UserService.get_detail_by_id(id=id, auth=auth)
...
async def create_obj(
    ...
    result_dict = await UserService.create_user(data=data, auth=auth)
...
async def update_obj(
    ...
    result_dict = await UserService.update_user(data=data, auth=auth)
...
async def delete_obj(
    ...
    await UserService.delete_user(id=id, auth=auth)
...
async def batch_set_available_obj(
    ...
    await UserService.set_user_available(data=data, auth=auth)
...
async def export_obj_template()-> StreamingResponse:
...
async def import_obj_list(
    file: UploadFile,
    auth: AuthSchema = Depends(AuthPermission(permissions=["system:user:import"]))
) -> JSONResponse:
    batch_import_result = await UserService.batch_import_user(file=file, auth=auth, update_support=True)
