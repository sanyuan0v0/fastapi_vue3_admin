...
from app.api.v1.services.autotest.case_service import CaseService
from app.api.v1.schemas.autotest.case_schema import CaseCreateSchema, CaseCreateSchema
...
async def get_obj_detail(
        ...
        auth: AuthSchema = Depends(AuthPermission(permissions=["autotest:case:query"])),
) -> JSONResponse:
    result_dict = await CaseService.get_detail_services(id=id, auth=auth)
...
async def get_obj_list(
        ...
        search: CaseQueryParams = Depends(),
        auth: AuthSchema = Depends(AuthPermission(permissions=["autotest:case:query"])),
) -> JSONResponse:
    result_dict_list = await CaseService.get_list_services(auth=auth, search=search, order_by=page.order_by)
...
async def create_obj(
        ...
        auth: AuthSchema = Depends(AuthPermission(permissions=["autotest:case:create"])),
) -> JSONResponse:
    result_dict = await CaseService.create_services(auth=auth, data=data)
...
async def update_obj(
        data: CaseCreateSchema,
        auth: AuthSchema = Depends(AuthPermission(permissions=["autotest:case:update"])),
) -> JSONResponse:
    result_dict = await CaseService.update_services(auth=auth, data=data)
...
async def delete_obj(
        id: int = Query(..., description="接口用例ID"),
        auth: AuthSchema = Depends(AuthPermission(permissions=["autotest:case:delete"])),
) -> JSONResponse:
    await CaseService.delete_services(auth=auth, id=id)
